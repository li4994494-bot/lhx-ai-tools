...
import { Copy, Github, ExternalLink, Sparkles, Video } from 'lucide-react';
...
const ProjectCard: React.FC<ProjectCardProps> = ({
  ...
  const isLingoMemo = project.type === 'lingo-memo';
    ...
    >
      ...
      <div className="relative aspect-[16/10] overflow-hidden bg-[#F2F2F7]">
        <img
          ...
          loading="lazy"
        />
        {(isInteractive || isVideoAnalyzer || isLingoMemo) && (
          <div className="absolute top-6 right-6 bg-white/90 backdrop-blur-md px-4 py-1.5 rounded-full flex items-center gap-2 border border-white/50 ios-shadow-sm">
            {isInteractive ? (
              ...
              <>
                <Sparkles size={12} className="text-[#6366f1] fill-[#6366f1]" />
                <span className="text-[10px] font-bold tracking-widest text-[#1d1d1f] uppercase">App</span>
              </>
            )}
          </div>
        )}
          ...
          <button
            ...
            className={`w-full flex items-center justify-center gap-2 py-4.5 rounded-[22px] text-[13px] font-bold transition-all ios-shadow active:scale-95 ${
              isInteractive
                ...
                : isLingoMemo
                ? 'bg-gradient-to-r from-[#6366f1] to-[#818cf8] text-white hover:brightness-110' // 靛蓝渐变
                : 'bg-[#1d1d1f] text-white hover:bg-black'
            }`}
          >
            <ExternalLink size={16} />
            {isInteractive ? '立即在线体验' : isVideoAnalyzer ? '打开丢帧检测工具' : isLingoMemo ? '打开翻译工具' : '获取项目链接'}
          </button>
        </div>
      </div>
    </motion.div>
