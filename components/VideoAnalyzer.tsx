...
const VideoAnalyzer: React.FC<VideoAnalyzerProps> = ({ isOpen, onClose }) => {
  ...
  const startAnalysis = async () => {
    ...
    video.onloadedmetadata = async () => {
      ...
      const totalEstimatedFrames = Math.floor(duration * targetFps);
      
      const bitrate = (file.size * 8) / duration;

      addLog(`视频信息加载成功: ${width}x${height}, ${duration.toFixed(2)}s, FPS设为: ${targetFps}`);
      ...
      const finishCurrentAnalysis = () => {
          finishAnalysis(frameCount, duplicateFrames, duration, targetFps, width, height, bitrate);
      };
      ...
      video.onseeked = () => {
        ...
        const nextTime = (nextFrameIndex + 0.5) / targetFps;

        if (nextTime < duration && frameCount < totalEstimatedFrames + 5) {
            setTimeout(() => {
      ...
      video.currentTime = 0.5 / targetFps;
    };
  };
